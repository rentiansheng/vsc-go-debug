...
interface variableInfo extends DebugProtocol.Variable {
	...
	addr: number;
	parentPath?: string;  // 父变量的路径，用于构建完整路径
	depth?: number;       // 嵌套深度
}

interface VariableInfo extends variableInfo {

	mapKeyInfo?: variableInfo; // map类型的key的变量信息
}
			...
			if (launchArgs.request === 'launch') {
				...
				if (launchArgs.noDebug) {
					if (mode === 'debug') {
						...
						this.debugProcess.on('close', (code: number) => {
							if (code) {
				...
				if (launchArgs.noCheckGoVersion) {
			...
			function connectClient(port: number, host: string, onClose?: Delve['onclose']) {
				...
				setTimeout(() => {
					...
					conn.on('connect', () => resolve(conn))
						.on('error', (err) => {
							if (dlvInstance.onstderr) {
								dlvInstance.onstderr?.(`Failed to connect to server: ${err}\n`);
								return;
							}
						})
						.on('close', (hadError) => {
...
export class GoDebugSession extends LoggingDebugSession implements EventEmitterMixin {
						...
						if (i % 2 === 0 && i + 1 < vari.children.length) {
							...
							return loadChildren(
								...
								return {
									...
									// 	value: mapKey.result,
									// 	evaluateName: vari.fullyQualifiedName + '[' + mapKey.result + ']',
									// 	variablesReference: mapKey.variablesReference,
									// 	type:  vari.children[i].type,
									// 	variablesReferenceCount: mapKey.variablesReferenceCount,
									// 	addr: vari.children[i].addr,
									// } as variableInfo,
								} as VariableInfo;
							});
						}
	...
	protected setVariableRequest(
		...
		if (args.value && args.value.includes('"')) {
			// 处理数组、切片、映射等复杂变量的赋值
			return this.setComplexVariableRequest(response, args);
		}
	...
	protected setComplexVariableRequest(
		response: DebugProtocol.SetVariableResponse,
		args: DebugProtocol.SetVariableArguments
	): void {

		const currentGoroutineId = this.debugState?.currentGoroutine.id || 0;
		const currentThreadId = this.debugState?.currentThread?.id || 0;
		const currentFrameId = 0; // 栈帧索引，0表示当前栈帧
        ...
        Frame: currentFrameId,
 			...
 			unsafe: false,
			expr: 'str = "zhangsanxxx"'
		};




		this.delve?.call('Command', [setVariableCommand], (err, out) => {
			if (err) {
				const errMessage = `Failed to set variable: ${err.toString()}`;
				this.logDelveError(err, 'Failed to set variable');
				return this.sendErrorResponse(response, 2010, errMessage);
	 ...
	 * 格式化变量值以便发送给 Delve
	 */
	private formatVariableValue(variable: DebugVariable, value: string): string {
		if (!variable) {
			return value;
		}
		if (!variable.kind) {
		...
		// 对于字符串，如果用户没有加引号，自动添加
		if (kind === GoReflectKind.String) {
			return `"${value}"`;
		} else if (kind === GoReflectKind.Bool) {
			// 对于布尔值，接受 true/false, 1/0
			if (value === '1') {
				return 'true';
			} else if (value === '0') {
				return 'false';
				...
				// 非法布尔值，返回原值
				GoDebugOutputProvider.Output(`"${variable.fullyQualifiedName}" value must be enclosed in square brackets []`, this.debugSession?.name);
				return value;
			}
			return value;
		} else if (kind === GoReflectKind.Map || kind === GoReflectKind.Slice || kind === GoReflectKind.Array) {
			// 对于 map, slice, array，用户需要输入类似 json 的格式
			// 例如 map[key1:val1 key2:val2], slice[val1 val2 val3], array[val1 val2 val3]
			// 这里做一个简单的验证，确保用户输入的值符合基本格式要求
			if (kind === GoReflectKind.Map) {
		...
		} else if (kind === GoReflectKind.Struct) {
			// 对于 struct，用户需要输入类似 json 的格式
			// 例如 {Field1:val1 Field2:val2}
			if (!value.startsWith('{') || !value.endsWith('}')) {
				GoDebugOutputProvider.Output(`❌ "${variable.fullyQualifiedName}" value must be enclosed in curly braces {}`, this.debugSession?.name);
				return value;
			}
			return `${variable.type}\{${value}\}`;
		}
	...
	private initLaunchAttachRequest(
		...
		this.delve.onstdout = (str: string) => {
			...
			if (this.debugSession) {
				GoDebugOutputProvider.Output(str, this.debugSession.name);
			}
	...
	private convertDebugVariableToProtocolVariable(v: DebugVariable): { result: string; variablesReference: number, variablesReferenceCount: number } {
		if (v.kind === GoReflectKind.UnsafePointer) {
			...
			// Default case - structs and other complex types
			if (v.children && v.children.length > 0) {
				// 为所有子变量生成正确的 fullyQualifiedName
				v.children.forEach((child) => {
					// 如果父变量有 fullyQualifiedName，使用它构建子变量的完整路径
					if (v.fullyQualifiedName) {
						child.fullyQualifiedName = `${v.fullyQualifiedName}.${child.name}`;
					} else if (v.name) {
						// 如果没有 fullyQualifiedName 但有 name，使用 name
						child.fullyQualifiedName = `${v.name}.${child.name}`;
					} else {
						// 最后的降级处理
